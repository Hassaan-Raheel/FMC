...
import './MetaAdsSummary.css'; // CSS file for styling
...
const MetaAdsSummary = ({ startDate, endDate }) => {
   ...
   useEffect(() => {
     const fetchAdData = async () => {
       try {
         ...
         const filteredAds = Array.isArray(metaAdsRes?.ads_data)
  ...
  const filteredAds = metaAds.filter((ad) => {
    const adDate = ad.insights[0]?.date_start; // Extract date from insights
    if (!adDate) return false;

    const adDateObj = new Date(adDate);
    const start = startDate ? new Date(startDate) : null;
    const end = endDate ? new Date(endDate) : null;

    return (!start || adDateObj >= start) && (!end || adDateObj <= end);
  });
  ...
  const aggregatedData = aggregateAdData(filteredAds);

  const chartData = filteredAds.map((ad) => {
    const insights = ad.insights[0] || {};

    return {
      campaign: ad.campaign_id || "Unknown",
